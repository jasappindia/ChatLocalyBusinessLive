...
public class PublishedServiceFragment extends Fragment {
    private RecyclerView rv_galleryView;
    private GridLayoutManager layoutManager;
    private PublishServiceListAdapter publishProductlistAdapter;
    List<ServiceListModel.PublishService> publishServiceList;
    private TextView tv_noProducts;

    public PublishedServiceFragment(List<ServiceListModel.PublishService> publishedProductList) {
        this.publishServiceList = publishedProductList;
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (publishServiceList != null)
            if (publishServiceList.size() > 0)
                tv_noProducts.setText(publishServiceList.size() + "    Service");
    ...
    public void setUserVisibleHint(boolean isFragmentVisible_) {
        ...
        if (this.isVisible()) {
            ...
            if (isFragmentVisible_ && !_hasLoadedOnce) {
                Constants.SERVICE_PUBLISHED_STATUS=Constants.PUBLISHED;
                tv_noProducts = (TextView) (getActivity()).findViewById(R.id.tv_noProducts);
                _hasLoadedOnce = true;
                searchView();
                if (publishServiceList != null)
    ...
    public void setRecycleView(View view) {
        ...
        publishProductlistAdapter = new PublishServiceListAdapter(getActivity(), publishServiceList);
    ...
    public class PublishServiceListAdapter extends RecyclerView.Adapter<PublishServiceListAdapter.MyViewHolder>  implements Filterable{
        private List<ServiceListModel.PublishService> filteredServiceList;
        private List<ServiceListModel.PublishService> serviceList;
        private Context context;

        public PublishServiceListAdapter(Context context, List<ServiceListModel.PublishService> productList) {
        ...
        @Override
        public PublishServiceListAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(context).inflate(R.layout.card_service, parent, false);

            return new PublishServiceListAdapter.MyViewHolder(view);
        }

        @Override
        public void onBindViewHolder(PublishServiceListAdapter.MyViewHolder holder, int position) {
            ...
            return new Filter() {
                ...
                protected FilterResults performFiltering(CharSequence charSequence) {
                    ...
                    if (str.isEmpty()) {
                        ...
                        ArrayList<ServiceListModel.PublishService> filterdList = new ArrayList<>();

                        for (ServiceListModel.PublishService product : serviceList) {
                ...
                protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                    filteredServiceList = (ArrayList<ServiceListModel.PublishService>) filterResults.values;
