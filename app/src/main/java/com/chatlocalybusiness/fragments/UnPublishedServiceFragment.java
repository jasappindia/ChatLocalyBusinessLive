...
public class UnPublishedServiceFragment extends Fragment{
    List<ServiceListModel.UnpublishService> unpublishServiceList;
    private RecyclerView rv_galleryView;
    private GridLayoutManager layoutManager;
    private UnpublishedServiceAdapter unpublishProductlistAdapter;
    private TextView tv_noProducts;

    public UnPublishedServiceFragment( List<ServiceListModel.UnpublishService> unpublishServiceList) {
        this.unpublishServiceList =unpublishServiceList;
    }
    ...
    public void setUserVisibleHint(boolean isFragmentVisible_) {
        ...
        if (this.isVisible()) {
            ...
            if (isFragmentVisible_ && !_hasLoadedOnce) {
                Constants.SERVICE_PUBLISHED_STATUS=Constants.UNPUBLISHED;

                tv_noProducts=(TextView)(getActivity()).findViewById(R.id.tv_noProducts);

                _hasLoadedOnce = true;
                searchView();
                if(unpublishServiceList !=null)
                    if(unpublishServiceList.size()>0)
                        tv_noProducts.setText(unpublishServiceList.size()+"    Service");
    ...
    public void setRecycleView(View view)
    {
        ...
        unpublishProductlistAdapter=new UnpublishedServiceAdapter(getActivity(), unpublishServiceList);
        rv_galleryView.setLayoutManager(layoutManager);
        rv_galleryView.setAdapter(unpublishProductlistAdapter);

    }
    public class UnpublishedServiceAdapter extends RecyclerView.Adapter<UnpublishedServiceAdapter.MyViewHolder> implements Filterable {
        private  List<ServiceListModel.UnpublishService> filteredServiceList;
        private  List<ServiceListModel.UnpublishService> serviceList;
        private Context context;
        private BusinessInfoModelNew.BusinessDetail info;

        public UnpublishedServiceAdapter(Context context, List<ServiceListModel.UnpublishService> serviceList) {
        ...
        @Override
        public UnpublishedServiceAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(context).inflate(R.layout.card_service, parent, false);

            return new UnpublishedServiceAdapter.MyViewHolder(view);
        }

        @Override
        public void onBindViewHolder(UnpublishedServiceAdapter.MyViewHolder holder, int position) {
...
//            holder.tv_proPrice.setText("Rs " + filteredServiceList.get(position).getServicePrice());
            if (filteredServiceList.get(position).getServiceStatus().equalsIgnoreCase("0"))
            ...
            return new Filter() {
                ...
                protected FilterResults performFiltering(CharSequence charSequence) {
                    ...
                    if (str.isEmpty()) {
                        ...
                        ArrayList<ServiceListModel.UnpublishService> filterdList = new ArrayList<>();

                        for (ServiceListModel.UnpublishService product : filteredServiceList) {
                ...
                protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                    filteredServiceList = (ArrayList<ServiceListModel.UnpublishService>) filterResults.values;
