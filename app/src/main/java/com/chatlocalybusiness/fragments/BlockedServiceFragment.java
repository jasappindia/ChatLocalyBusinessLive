...
public class BlockedServiceFragment extends Fragment{
    List<ServiceListModel.BlockedService> blockedServiceList;
    private RecyclerView rv_galleryView;
    private GridLayoutManager layoutManager;
    private BlockedServiceListAdapter blockedProductListAdapter;
    private TextView tv_noProducts;

    @SuppressLint("ValidFragment")
    public BlockedServiceFragment(List<ServiceListModel.BlockedService> blockedServiceList) {
        this.blockedServiceList=blockedServiceList;
    ...
    public void setUserVisibleHint(boolean isFragmentVisible_) {
        ...
        if (this.isVisible()) {
            ...
            if (isFragmentVisible_ && !_hasLoadedOnce) {
                ...
                _hasLoadedOnce = true;
                searchView();
                if(blockedServiceList!=null)
                    if(blockedServiceList.size()>0)
                        tv_noProducts.setText(blockedServiceList.size()+"    Service");
    ...
    public void setRecycleView(View view)
    {
        ...
        blockedProductListAdapter=new BlockedServiceListAdapter(getActivity(),blockedServiceList);
        rv_galleryView.setLayoutManager(layoutManager);
        rv_galleryView.setAdapter(blockedProductListAdapter);
    }
    public class BlockedServiceListAdapter extends RecyclerView.Adapter<BlockedServiceListAdapter.MyViewHolder> implements Filterable {
        private List<ServiceListModel.BlockedService> filteredServiceList;
        private List<ServiceListModel.BlockedService> serviceList;
        private Context context;
        private BusinessInfoModelNew.BusinessDetail info;

        public BlockedServiceListAdapter(Context context, List<ServiceListModel.BlockedService> productList) {
        ...
        @Override
        public BlockedServiceListAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(context).inflate(R.layout.card_service, parent, false);

            return new BlockedServiceListAdapter.MyViewHolder(view);
        }

        @Override
        public void onBindViewHolder(BlockedServiceListAdapter.MyViewHolder holder, int position) { holder.tv_productName.setText(filteredServiceList.get(position).getServiceName());
            holder.tv_proPrice.setText("Rs " + filteredServiceList.get(position).getServicePrice());
            if (filteredServiceList.get(position).getStatus().equalsIgnoreCase("BLOCKED"))
            ...
            return new Filter() {
                ...
                protected FilterResults performFiltering(CharSequence charSequence) {
                    ...
                    if (str.isEmpty()) {
                        ...
                        ArrayList<ServiceListModel.BlockedService> filterdList = new ArrayList<>();

                        for (ServiceListModel.BlockedService product : serviceList) {
                ...
                protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                    filteredServiceList = (ArrayList<ServiceListModel.BlockedService>) filterResults.values;
